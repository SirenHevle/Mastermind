...
export class TMenu {
  ...
  onCheckAnswerClick = () =>{
    //Denne sjekker om det er valgt rett farge
    ...
    //Sjekke om det er valgt riktig farge på riktig plass
    ...
    //Sjekke om det er valgt riktig farge på feil plass.
        ...
        for(let j = 0; j < 4; j++){
          ...
          if(computerAnswer.checkThis && (playerAnswer.pos !== computerAnswer.pos)){
            if(playerAnswer.color === computerAnswer.color){
              ...
              //  Må ikke sjekke disse to fargene igjen
              computerAnswer.checkThis = playerAnswer.checkThis = false;
            }
          }
  ...
  #createColorHint(posIndex, colorIndex){
    ...
    return posIndex; // Returnere den nye indeksen til posisjonen
